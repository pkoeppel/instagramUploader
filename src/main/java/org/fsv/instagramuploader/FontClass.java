package org.fsv.instagramuploader;

import java.awt.*;

public class FontClass {
	public static final Font headMen1 = new Font("Dialog", Font.BOLD, 190);
	public static final Font headMen2 = new Font("Dialog", Font.BOLD, 130);
	public static final Font headMen3 = new Font("Dialog", Font.BOLD, 90);
	public static final Font clubMen = new Font("Dialog", Font.BOLD, 30);
	public static final Font bottomMen = new Font("Dialog", Font.BOLD, 34);
	public static final Font headYouth1 = new Font("Calibri", Font.BOLD, 100);
	public static final Font teamYouth = new Font("Comic Sans MS", Font.BOLD, 40);
	public static final Font simpleYouth = new Font("Comic Sans MS", Font.PLAIN, 24);
	public static final Font resultYouth = new Font("Comic Sans MS", Font.PLAIN, 35);
	public static final Font clubOwnYouth = new Font("Comic Sans MS", Font.BOLD, 24);
	public static final Font mTypeYouth = new Font("Comic Sans MS", Font.BOLD, 33);
	
}
